...
func NormalizeInboundEndpoint(path string) string {
	...
	case strings.Contains(path, EndpointRealtime) || isBareOrSubpathOf(strings.TrimRight(path, "/"), "/realtime"):
...
func DeriveUpstreamEndpoint(inbound, rawRequestPath, platform string) string {
	...
	case service.PlatformOpenAI, service.PlatformGrok:
		...
		if inbound == EndpointEmbeddings || inbound == EndpointAlphaSearch || inbound == EndpointRealtime || inbound == EndpointResponsesInputTokens || inbound == EndpointImagesGenerations || inbound == EndpointImagesEdits || inbound == EndpointVideosGenerations || inbound == EndpointVideosEdits || inbound == EndpointVideosExtensions || inbound == EndpointVideos {

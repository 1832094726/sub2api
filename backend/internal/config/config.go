...
func setDefaults() {
	...
	viper.SetDefault("chatgpt2api_image.group_ids", []int64{})
...
func (c *Config) Validate() error {
	...
	if c.ChatGPT2APIImage.PrimaryEnabled {
		seenGroupIDs := make(map[int64]struct{}, len(c.ChatGPT2APIImage.GroupIDs))
		for _, groupID := range c.ChatGPT2APIImage.GroupIDs {
			if groupID <= 0 {
				return fmt.Errorf("chatgpt2api_image.group_ids must contain positive IDs")
			}
			if _, exists := seenGroupIDs[groupID]; exists {
				return fmt.Errorf("chatgpt2api_image.group_ids must not contain duplicates")
			}

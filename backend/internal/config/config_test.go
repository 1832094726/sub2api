...
func TestChatGPT2APIImageDefaults(t *testing.T) {
	...
	require.Empty(t, cfg.ChatGPT2APIImage.GroupIDs)
...
func TestChatGPT2APIImageGroupIDsFromEnvironment(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("CHATGPT2API_IMAGE_PRIMARY_ENABLED", "true")
	t.Setenv("CHATGPT2API_IMAGE_GROUP_IDS", "12,34")
	t.Setenv("CHATGPT2API_IMAGE_BASE_URL", "http://127.0.0.1:3000")
	t.Setenv("CHATGPT2API_IMAGE_API_KEY", "secret")

	cfg, err := Load()
	require.NoError(t, err)
	require.Equal(t, []int64{12, 34}, cfg.ChatGPT2APIImage.GroupIDs)
}

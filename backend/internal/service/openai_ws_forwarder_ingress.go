...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	if forceHTTPBridge || s.shouldBridgeOpenAIWSHTTP(account, firstPayload.payloadBytes, firstPayload.previousResponseID) {
		...
		for turn := 1; ; turn++ {
			...
			if responseID != "" && stateStore != nil && result.SucceededForScheduling() {
	...
	for {
		...
		if relayErr != nil {
			lastTurnClean = false
			if isOpenAIWSSessionPreempted(ctx) {
				if sessionLease != nil && !primaryHandled {
					sessionLease.MarkBroken()
				}
				return errOpenAIWSSessionPreempted
			}
			var rejectedFieldErr *openAIWSRejectedFieldRetryError
			if !primaryHandled && errors.As(relayErr, &rejectedFieldErr) && rejectedFieldErr != nil && len(rejectedFieldErr.body) > 0 {
				currentPayload = append([]byte(nil), rejectedFieldErr.body...)
				currentPayloadBytes = len(currentPayload)
				skipBeforeTurn = true
				continue
			}
			if !primaryHandled && recoverIngressPrevResponseNotFound(relayErr, turn, connID) {
				continue
			}
			if !primaryHandled && retryIngressTurn(relayErr, turn, connID) {
				continue
			}
			finalErr := relayErr
		...
		if responseID != "" && stateStore != nil && !primaryHandled && result.SucceededForScheduling() {
			ttl := s.openAIWSResponseStickyTTL()
			logOpenAIWSBindResponseAccountWarn(groupID, account.ID, responseID, stateStore.BindResponseAccount(ctx, groupID, responseID, account.ID, ttl))

...
func (s *AccountTestService) testOpenAICompactConnection(c *gin.Context, account *Account, testModelID string) error {
	...
	if isOAuth {
		...
		if account.GetCodexFingerprintMode() == codexFingerprintSession {
			identity := extractCodexFingerprintIngressIdentity(req.Header, payloadBytes)
			if identity.client.sessionID != "" {
				fpIDs = resolveCodexFingerprintConversationRootIDs(
					account,
					0,
					identity.client,
					hashCodexConversationValue("session", identity.client.sessionID),
				)
			}
		}
		if fpIDs != nil {

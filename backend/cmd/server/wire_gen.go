...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	accountImportSnapshotRepository := repository.NewAccountImportSnapshotRepository(client)
	accountImportSnapshotService := service.NewAccountImportSnapshotService(accountImportSnapshotRepository, secretEncryptor)
	accountHandler := admin.ProvideAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService, accountImportSnapshotService)
	...
	imagePrimaryClient, err := service.ProvideChatGPT2APIImageClient(configConfig, settingService)
	if err != nil {
		return nil, err
	}
	imagePrimaryTaskRepository := repository.NewImagePrimaryTaskRepository(db)
	imagePrimaryRouter := service.NewImagePrimaryRouter(imagePrimaryClient, imagePrimaryTaskRepository, configConfig)
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, pluginManager, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, imagePrimaryRouter, grokQuotaService, configConfig, coordinator)

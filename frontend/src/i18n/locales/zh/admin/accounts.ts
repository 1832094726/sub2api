export default {
    accounts: {
      ...
      dataImportWarning: '导入将创建新账号与代理，并按下方默认配置绑定账号；请确认已有数据不会冲突。',
      dataImportFile: '数据文件',
      dataImportDefaultProxy: '默认代理',
      dataImportDefaultGroup: '默认分组（仅绑定同平台账号）',
      dataImportNoDefaultGroup: '不指定默认分组',
      dataImportDefaultPriority: '默认优先级',
      dataImportOpenAIPassthrough: 'OpenAI 自动透传',
      ...
      healthCheck: {
        action: '批量测活',
        title: '账号批量测活',
        description: '通过账号当前配置和代理发送真实测试请求，认证失败会同步更新账号状态。',
        targetCount: '本次将测试 {count} 个账号',
        progress: '执行进度',
        completed: '已完成',
        success: '成功',
        failed: '失败',
        ...
        start: '开始测活',
        runAgain: '重新测活',
        noAccounts: '当前范围内没有可测试的账号',
        loadTargetsFailed: '加载测活账号失败'
      },
      bulkActions: {
        ...
        refreshTokenSummary: '刷新并验证成功 {verified} 个；缺少刷新令牌 {missing} 个；Refresh Token 已失效 {invalidated} 个；令牌类型不支持 {unsupported} 个；代理或网络失败 {transport} 个；上游未返回访问令牌 {empty} 个；令牌未变化 {unchanged} 个；刷新后验证失败 {unverified} 个；其他失败 {failed} 个',
      ...
      openai: {
        ...
        codexFingerprintModeDesc: '多人共享同一 OAuth 账号时，设备标识保持账号级稳定；“设备 + 有界会话”会为显式逻辑对话保留独立根，并把完全无身份的 API 请求限制在 8–32 个租约会话内。Codex 根会把所选上游账号至少粘连 24 小时（使用时续期），不受调度优先级/权重变化迁移，临时并发拥堵时等待而不是溢出到其他账号。每个根同时只运行一个 Turn，支持 Codex 客户端断开或 /responses/{id}/cancel 中断；被中断的响应不会成为续接锚点。默认关闭（原样透传客户端标识），需要时再显式开启；部分账号开启收敛后出现过额度缩水，请按自己的实测结果选择。',
        codexFingerprintOff: '关闭（透传，默认）',
        codexFingerprintDevice: '仅设备',
        codexFingerprintSession: '设备 + 有界会话',

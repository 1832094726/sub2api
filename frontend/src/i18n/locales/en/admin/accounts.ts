export default {
    accounts: {
      ...
      dataImportWarning: 'Import creates accounts and proxies and applies the defaults below. Ensure existing data does not conflict.',
      dataImportFile: 'Data file',
      dataImportDefaultProxy: 'Default Proxy',
      dataImportDefaultGroup: 'Default Group (matching platform only)',
      dataImportNoDefaultGroup: 'No default group',
      dataImportDefaultPriority: 'Default Priority',
      dataImportOpenAIPassthrough: 'OpenAI automatic passthrough',
      ...
      healthCheck: {
        action: 'Health Check',
        title: 'Bulk Account Health Check',
        description: 'Send real test requests through each account and its configured proxy. Authentication failures update account status.',
        targetCount: '{count} account(s) will be tested',
        progress: 'Progress',
        completed: 'Completed',
        success: 'Success',
        failed: 'Failed',
        ...
        start: 'Start',
        runAgain: 'Run Again',
        noAccounts: 'No accounts match the current scope',
        loadTargetsFailed: 'Failed to load health-check targets'
      },
      bulkActions: {
        ...
        refreshTokenSummary: 'Refreshed and verified: {verified}; missing refresh token: {missing}; invalidated Refresh Token: {invalidated}; unsupported token type: {unsupported}; proxy or network failures: {transport}; empty access token: {empty}; token unchanged: {unchanged}; verification failed: {unverified}; other failures: {failed}',
      ...
      openai: {
        ...
        codexFingerprintModeDesc: 'For a shared OAuth account, the device identifier stays account-stable. “Device + bounded sessions” preserves a separate root for explicit logical conversations and confines identity-free API calls to an 8–32 slot leased pool. Codex roots pin the selected upstream account for at least 24 hours (refreshed on use), keep hard affinity across scheduler priority/weight changes, and wait instead of spilling to another account under temporary concurrency pressure. Each root runs one Turn at a time and can be interrupted by an official Codex client disconnect or /responses/{id}/cancel; interrupted responses never become continuation anchors. Off by default (client identifiers pass through as-is); opt in explicitly when needed. Some accounts reported quota shrinkage after enabling convergence, so choose based on your own measurements.',
        codexFingerprintOff: 'Off (passthrough, default)',
        codexFingerprintDevice: 'Device only',
        codexFingerprintSession: 'Device + bounded sessions',
